...
## 3.6. 意図的な先送り（deferred scope / REQUIRED）

> **背景 (Sess99 / Issue #1119-#1124)**: Sess93 で PR-5/PR-7 を「follow-up」と PR 本文 + ADR にのみ
> 記録して Issue 未起票 → 追跡消失し、user が v13 実機検証で「直っていない」と発見した。
> 先送りは **PR 本文 / ADR に書くだけでは完了と認めない** — Issue # 記入で初めてチェック可。

- [ ] 本 PR で **意図的に scope 外へ先送りした項目** を以下に列挙した（なければ「なし」と記載）:
  - なし
- [ ] 先送り項目ごとに **Issue # を記入した**（Issue なしの先送りは禁止。例: work-picker 確認画面化 → #1119）
- [ ] 先送り項目が **user が議論で要望した内容** を含む場合、セッション完了報告で「やらなかったこと」として明示する
...
> **背景**: Sess72 scroll 復元で「直した」 とリリースノート告知後、 テスターから「維持されず TOP に戻る」 と再報告 (Sess95)。 完了判定が Jest + lint のみで、 報告された再現手順そのものの実機再現確認が gate に含まれていなかった。

- [ ] テスター報告の **再現手順をそのまま実機でなぞった** SS / 動画を PR 本文 or コメントに添付
- [ ] リリースノートに「修正済」 と書くのは上記証拠がある項目のみ
- [ ] 適用対象外の場合: [ ] 本 PR はテスター報告起点の bug 修正を含まない
