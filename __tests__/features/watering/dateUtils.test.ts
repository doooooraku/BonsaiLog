/**
 * dateUtils.ts 純関数テスト (PR-A、ADR-0039 前段の安全 refactor)。
 *
 * 旧 `wateringHeatmap.test.ts` から shared util テスト
 * (toLocalDateKey / diffDayKeys / getLastWatering / getDaysSinceLastWatering / classifyLastWatered)
 * を切り出し、import path のみ変更 (テスト内容は無変更)。
 */
import {
  classifyLastWatered,
  diffDayKeys,
  ...
  getLastWatering,
  toLocalDateKey,
} from '@/src/features/watering/dateUtils';
