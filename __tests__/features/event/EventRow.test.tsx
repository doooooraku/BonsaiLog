...
describe('EventRow 改善 ② (ADR-0036 D9、 showBonsaiName=true 時 重複削除)', () => {
  // Sess66 PR6c.1: dark cascade で `style={styles.X}` → `style={[styles.X, { color: c.* }]}` に変更されたため、
  // 正規表現を `style=\{(\[?)styles\.X` 形式 (単独 / 配列両対応) に緩和。
  test('1. showBonsaiName=true 分岐は bonsaiName 単独 (eventLabel + date を render しない)', () => {
    ...
    // Sess81 PR-7 で <View style={styles.titleWithRecurring}> wrapper 追加 (= 🔁 RepeatIcon 並列)、
    // 正規表現を View wrapper 任意で 緩和。
    expect(COMPACT_SRC).toMatch(
      /showBonsaiName\s*&&\s*bonsaiName\s*\?\s*\(\s*(?:<View[^>]*titleWithRecurring[^>]*>\s*)?<ThemedText\s+style=\{[[\s]*styles\.eventBonsaiName/,
    );
  });
  ...
  test('2. showBonsaiName=false 分岐は eventLabel + eventRowDate を Fragment で render (bonsai-detail 維持)', () => {
    ...
    // Sess81 PR-7: eventLabel は <View style={styles.titleWithRecurring}> wrapper 内に移動 (= 🔁 並列)、
    // Fragment 直下 → View 直下 の構造変化。 regex を View 経由許容に緩和。
    expect(COMPACT_SRC).toMatch(
      /<>\s*\n\s*(?:<View[^>]*titleWithRecurring[^>]*>\s*\n?\s*)?<ThemedText\s+style=\{[[\s]*styles\.eventLabel/,
    );
    expect(COMPACT_SRC).toMatch(/<ThemedText style=\{[[\s]*styles\.eventRowDate/);
  });
  ...
  test('3. 旧 「showBonsaiName=true 時に eventRowDate も同時 render」 が削除済', () => {
    ...
    // Sess81 PR-7 で <View style={styles.titleWithRecurring}> wrapper 追加で eventRowMain 内に
    // ネストした View が増えたため、 旧 lazy regex (= 内側 </View> で 切れる) は機能不全。
    // 簡略化: source 全体での `styles.eventRowDate` 出現回数 = 1 を assert (= showBonsaiName=false 分岐のみ)。
    const occurrences = COMPACT_SRC.match(/styles\.eventRowDate/g);
  ...
  test('4. 旧 「showBonsaiName=true 時 eventLabel 重複 ThemedText」 が削除済', () => {
    ...
    expect(COMPACT_SRC).not.toMatch(
      /showBonsaiName\s*&&\s*bonsaiName\s*&&\s*\(\s*\n?\s*<ThemedText style=\{[[\s]*styles\.eventLabel/,
    );
  });
