...
| ID  | 違反        | 実ファイル:行                                             | 是正                                                                             |
| --- | ----------- | --------------------------------------------------------- | -------------------------------------------------------------------------------- |
| F1a | core→stores | `src/core/theme/useColors.ts:16`（`useSettingsStore` 値） | useColors は core 据え置き、core→stores を例外受容（PR 6-3、ADR-0048 Amendment） |
...
- 移設（実体 + re-export 凍結）: `src/types/units.ts`★ / `src/db/eventPayloadValidator.ts`★ / `src/features/photos/photoOrchestrator.ts`★
- 据え置き + 例外受容: `src/core/theme/useColors.ts`（F1a、`core→stores` を eslint-disable、ADR-0048 Amendment）
...
```
Before                                  After
src/core/theme/useColors.ts (実体)      src/core/theme/useColors.ts (据え置き) ★
                                        └ core→stores 1本を eslint-disable で受容
...
| **6-3** | F1a: useColors は core 据え置き、`core→stores` 1 本を eslint-disable + ADR-0048 Amendment で受容（3 部品が使用のため移設不可） | 3→2        | 低（コードロジック不変）                            |
...
- **Q1 useColors の配置**: ~~features/theme~~ → 実装時に components 3 部品が使用と判明し移設不可。**core 据え置き + 例外受容**に決定（ADR-0048 Amendment 2026-05-29、user 承認済）。
