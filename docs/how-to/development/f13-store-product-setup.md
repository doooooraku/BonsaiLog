...
>
> ⚠️ **現行の正 (2026-06-12 注記)**: ストア商品の作成は **ADR-0043 の API 自動化 (`scripts/store-automation/`) が正式ルート** (実商品も API で作成済)。本書の GUI 手順は手動 fallback。Product ID は **`bonsailog_pro_annual`** (`_yearly` は ADR-0009 Sess47 Amendment で誤記と確定)、Google は subscription `bonsailog_pro` (basePlan `monthly`/`annual`) + onetimeproduct 構成、価格は **USD 基準** ($3.99/$29.99/$69.99、各国自動換算 ≈ ¥600/¥3,980/¥9,800)。チェックリストは `docs/how-to/release/iap-setup-checklist.md` が最新。
...
[Apple App Store Connect]              [Google Play Console]
    ...
    bonsailog_pro_annual         +         bonsailog_pro_annual
...
3. **年額**:
   - Product ID: `bonsailog_pro_annual`
   - Reference Name: `BonsaiLog Pro Annual`
   - 期間: 1 Year
   - 価格: ¥3,980 (Tier 40 相当)
...
2. **年額**:
   - Product ID: `bonsailog_pro_annual`
   - Name: `BonsaiLog Pro Annual`
   - Base Plan: `annual` (Auto-renewing、1 Year)
   - 価格: ¥3,980
...
- `bonsailog_pro_annual` → Entitlement: `premium`
...
- Packages:
  ...
  - `$rc_annual` → Product `bonsailog_pro_annual` (RevenueCat 規約上 `annual` キー、`yearly` ではない)
