...
A. species / style / tags 入力済の盆栽を 1〜2 件、実機で手動作成してから再 PoC を実行する。データ問題と実装問題の混同を避けるため。seed は `src/dev/seedTestData.ts` (設定タブ → 開発者セクション、11 盆栽 1 タップ投入) が実装済みなのでそれを使う。
