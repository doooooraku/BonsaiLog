...
| P-09 | [P-09_ui-diff-pipeline.md](P-09_ui-diff-pipeline.md)                       | Claude Code   | ADR-0021 UI 差分検出パイプライン (= 1 語トリガー、 画面比較 → Issue → 実装 → merge) |
| P-10 | [P-10_open-design-resume.md](P-10_open-design-resume.md)                   | Claude Code   | open-design 再開 (= 1 語トリガー、 安定タグ確認 → 起動 → URL 報告)                  |
| P-11 | [P-11_implement-from-mockups.md](P-11_implement-from-mockups.md)           | Claude Code   | docs/mockups/v1.0/ → React Native 実装 (R-28 境界判定厳守、 P-09 と併用)            |
| P-12 | [P-12_screen-implementation-cycle.md](P-12_screen-implementation-cycle.md) | Claude Code   | 画面実装 cycle (= ADR-0021 と組み合わせ)                                            |
...
- **2026-06-10 (Sess91 Phase 1c、 PR #1046 想定)**: P-02〜P-08 7 件 retire。 30 日 0-reads + ClaudeCode で 全 file 内容確認 + 7 分類判定 (A 役目終) で確定。 後継:
  ...
  → keep 4 件 (P-00 / P-01 / P-09 / P-10) + P-11 / P-12 は 0 reads でないため retire 対象外。 ADR-0046 D-2 「物理削除禁止」 は ADR/R ルール対象、 prompt は git history で revert 可能なので物理削除採用。
