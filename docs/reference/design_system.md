...
> 注: 屋外モードは ADR-0015 Notes Amended 2026-05-10 で **撤去済** (実装なし)。 上表は歴史的参考として温存。
...
1. **ESLint custom rule** `local/no-color-token-in-stylesheet` (`eslint-rules/no-color-token-in-stylesheet.js`)
   ...
   - **Sess70 PR-D (完了)**: FORBIDDEN_TOKENS に brand-static 8 種追加済 (上記撤回リスト)
2. **ESLint rule (Sess70 PR-D で導入済)** `local/no-color-hex-literal-in-stylesheet` (`eslint-rules/no-color-hex-literal-in-stylesheet.js`)
...
5. **R-59 + R-60 (Sess70 PR-D 起票済)**: brand tokens scheme-aware 必須 / 新画面 PR は dark SS 添付必須
...
| `screenTitleStack` | `NotoSerifJP_500Medium` | 18       | — (型制限) | — (型制限)    | Stack 画面 ヘッダー (React Navigation native)    |
...
> **Note**: §18 が本ファイル内で 2 重採番されている (「アンチパターン」 と 「長押し UX 標準」)。 多数の ADR / lessons が「§18 長押し UX 標準」 を参照しているため、 採番はこのまま維持する (2026-06-12 Sess103 監査で確定 — 旧 Note の「別 PR fix 予定」 は参照破壊リスクのため見送り)。
...
> ⚠️ **注記 (2026-06-12 Sess103 監査)**: `BADGE_SOFT_BG` / `BADGE_SOFT_TEXT` の StyleSheet 内 static 直接参照は Sess69 PR-A で撤回済 (§2-4 撤回リスト)。 現行は inline `c.badgeBg` / `c.tint` 経由 (dark = `#2C3329` / `#7FA98A`)。 本節の hex 値は light モードの定義として有効。
...
> ⚠️ **注記 (2026-06-12 Sess103 監査)**: `BRAND_GREEN` / `BUTTON_SECONDARY_BG` / `BUTTON_SECONDARY_TEXT` の StyleSheet 内 static 直接参照は Sess69 PR-A で撤回済 (§2-4 撤回リスト)。 現行は inline `c.tint` / `c.onTint` / `c.buttonSecondaryBg` 経由 (dark = `#7FA98A` / `#1A1A1A` / `#2C3329`)。 本節の hex 値は light モードの定義として有効。
...
_このドキュメントは `src/core/theme/colors.ts` (light) + `constants/theme.ts` (light/dark map) として TypeScript 定数にも反映される。整合は `pnpm theme:sot` が機械検証する。_
_変更時は ADR (`docs/adr/ADR-XXXX-<slug>.md` 形式) を作成。_
