...
- 自前の Development Build (`com.dooooraku.bonsailog`) は Dev Launcher を持っているが、`exp://` を取られてしまう。
...
## 5. CI polling の fail 検出漏れ + GitHub Actions runner 取得失敗

### 症状

- `gh pr checks <pr> --watch` が Bash tool で予期せず background 化されることがある(出力ファイル経由でしか結果が見えない)。
- 自前 polling を `grep -q 'pass'` だけで書くと、**`fail` を検出できず timeout までブロック**する。
- GitHub Actions の `docs-cleanliness` job が runner 取得失敗で **15 分 pending → fail** になることがある(コード問題ではなく一時的なインフラ flake)。

### 構造的対処

**`scripts/ci/wait-for-pr.sh <pr-num>`** を使う:

```bash
bash scripts/ci/wait-for-pr.sh 123
```

- 内部で `gh pr checks` を polling
- `fail` / `pending` / `pass` を区別して判定
- timeout (default 6 分) 超過で **自動 cancel + `gh run rerun --failed`** を最大 2 回試行(runner flake 自動吸収)
- exit code: 0 = pass / 1 = real failure / 2 = max retries exceeded

### 検出方法

PR 作成後、`gh pr checks --watch` を直接呼ばず必ず本スクリプト経由で。Claude Code の Bash tool で background 化されても、polling ロジックは foreground で完走する設計。

---

## 6. 再 PoC 時の一過性 Empty + Error(force-stop 必要)

### 症状

ADR-0021 パイプラインで 2 度目以降の `run.ts` 実行時、まれに以下が発生:

- アプリが Empty 状態(盆栽 0 件)で表示
- エラートースト `Uncaught (in promise, id: 0) Error: Call to function...`
- 1 度目の PoC は正常、`adb shell am force-stop` + 再起動で解消

### 原因(推定)

- SQLite native call (expo-sqlite) と RN navigator (expo-router の Tabs) 初期化のタイミング race
- Metro バンドル切り替え直後に DB クエリが走り、native module 状態が中途半端になる

### 構造的対処

**`scripts/ui-diff/capture-app.sh` の冒頭(preflight 後、Maestro 起動前)で必ず force-stop を実行**:
...
これにより、再 PoC 時もクリーンなアプリ起動が保証される。本セッション PR で組み込み済。
