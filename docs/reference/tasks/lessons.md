...
> **このファイルは「技術 lesson」専用です**（DB / Backup / ビルド / マイグレ等の領域別）。
>
> 「**行動 lesson**」（一括処理後の目視確認、履歴は ADR 集約、ペルソナ評価必須 等）は **`.claude/recurrence-prevention.md`** に集約されています。
> 行動パターンの再発防止は recurrence-prevention.md を参照してください。
