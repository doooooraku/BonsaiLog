...
- **この文書に「書かない」こと**（ドリフト防止）:
  - **コード例・擬似コード** → 実装ファイルが正 (「実装の正: <path>」 pointer のみ置く。埋め込みコードは腐る — Sess101 監査で TanStack/i18next/Tamagui の虚構コードを大量検出した教訓)
...
| F-15   | テーマ (システム/ライト/ダーク) | §20    |
| F-16   | ローカル通知                    | §21    |
| F-17   | 作業カレンダー                  | §23    |
| F-26   | 文脈内ガイド（オンボ拡張）      | §26    |
| —      | 定期予定 (F-02 拡張、ADR-0056)  | §27    |
| —      | カスタム樹種・樹形 (ADR-0026)   | §6.3.6 |
...
- **reload（再読込）**: 書き込み完了後、画面側が repository を再度呼んで表示データを取り直す操作（TanStack Query は Phase 7 PR #898 で撤去済みのため、キャッシュ層は持たない）。
...
AppState → active 時に TZ 変化を検知したら通知を現地時刻で再予約する (F-16、ADR-0014 B1)。実装の正: `app/_layout.tsx`。

### §5.3 状態管理レイヤー（3 層分離、Phase 7 で TanStack Query 撤去後の現行構成）

| レイヤー         | 役割                             | 技術                                               |
| ---------------- | -------------------------------- | -------------------------------------------------- |
| **永続・真実**   | 正データ                         | expo-sqlite (WAL)                                  |
| **データ供給**   | 画面への読み出し                 | `src/db/*Repository.ts` を画面側 hook から直接呼ぶ |
| **UI・ドラフト** | 揮発性選択状態・設定・Pro 状態等 | Zustand v5 (+ AsyncStorage persist)                |

**原則**: SQLite が source of truth。キャッシュ層は持たず、書き込み後は呼び出し元が **reload（repository 再読込）** で同期する。画面復帰時は `useFocusEffect` 等で再取得。

**絶対ルール**:

- SQLite データを Zustand に丸ごとコピーしない（二重管理アンチパターン）
- Zustand は UI 状態（選択中 ID、フィルタ、ドラフト）と軽量設定のみ
- 複数フィールド選択時は `useShallow` を使う（v5 破壊的変更）
- 書き込み成功・失敗に関わらず、表示は reload で DB と一致させる
...
### §5.5 書き込み後の再読込ルール（旧 invalidateQueries 表の置き換え）

TanStack Query 撤去後の同期は単純化されている:

- **同一画面内の書き込み**: 保存処理の完了後に同画面の `reload()`（repository 再取得）を呼ぶ
- **別画面への反映**: 遷移先・復帰先の画面が `useFocusEffect` / mount 時に repository から取得する
- **お引っ越し復元 (F-11)**: 復元完了後に全画面が次回 focus で再取得（キャッシュが無いため特別な破棄処理は不要）
- **Pro 購入 (F-13)**: `proStore`（Zustand）が `CustomerInfoUpdateListener` 経由で即時反映
- **言語切替 (F-12)**: i18n store の更新で全 `t()` 購読コンポーネントが再レンダ

### §5.6 楽観的更新は不採用

optimistic update（先に UI を書き換えて後で DB 反映）は **採用しない**。書き込み → reload の同期更新を標準とし、取り消し可能な操作は **softDelete + UndoSnackbar + restore**（ADR-0036）で実現する。
...
- label は短く (目安 14 字以内、19 言語)、action は動詞 (「元に戻す」等)
- 表示時間は約 4〜5 秒、同時複数は**後発で置換** (スタックしない)
- 実装の正: `src/components/Toast.tsx` (UndoSnackbar = action slot 拡張、ADR-0036)

### §5.10 データの正（DB）と画面状態

- **正**: ローカル DB（SQLite）
- **画面表示用**: repository 読み出し結果の component state（キャッシュ層なし、§5.3）
- **UI ドラフト**: Zustand
- **原則**: 永続化は DB で確実に、表示は reload で DB と一致させる
...
実装の正: `src/db/schema.ts` (CREATE TABLE IF NOT EXISTS で冪等、ALTER 前のカラム存在チェック、user_version は migration 末尾で無条件 set)。

### §5.12 PRAGMA 初期化（起動時必須）

`journal_mode=WAL` (並行読み書き) / `synchronous=NORMAL` / `foreign_keys=ON` (既定 OFF) / `busy_timeout=5000` — 実装の正: `src/db/schema.ts` の初期化処理。
...
```mermaid
flowchart TD
  Home[Home画面] -->|BottomCtaBar「追加」タップ| Modal[bonsai-new モーダル]
  Modal -->|樹種選択| Species[species-picker modal]
  Species -->|選択| Modal
  Modal -->|写真追加| Camera[expo-image-picker]
  Camera --> Modal
  Modal -->|Save| Validate{バリデーション}
  Validate -- NG --> Error[インラインエラー]
  Validate -- OK --> TXN[repository 経由 INSERT]
  TXN -->|成功| DismissReplace[dismissAll + replace /bonsai/:id]
  DismissReplace --> Detail[盆栽詳細]
  TXN -->|失敗| RollbackError[UI エラー表示]
```
...
実装の正は `src/db/bonsaiRepository.ts`（`updateBonsai` 等）。画面側は repository を直接呼び、完了後に reload (§5.5) して戻る。

#### §6.3.4 アーカイブ / 復元

- アーカイブは `archived_at` に現在時刻を書く（物理削除しない）
- Home 一覧は `WHERE archived_at IS NULL` でフィルタ
- アーカイブ画面（設定 → アーカイブ済み盆栽 `app/settings/archived.tsx`）から復元可能
- **完全削除**はアーカイブ済みの盆栽に対してのみ可能（`ConfirmDialog` で確認、ADR-0036。タイプ入力方式は不採用）
- 完全削除時: CASCADE で `events`, `photos`, `recurrence_rules` 等の子データも削除
- 完全削除時の写真ファイルは物理削除

#### §6.3.5 並び替え

- 一覧の並び順は **`updated_at DESC` 固定**（`bonsaiRepository.ts`）
- 手動ドラッグ並び替え・ソート切替は **v1.0 未実装**（将来候補。当初構想の `bonsai_order` テーブルは作られていない）

#### §6.3.6 カスタム樹種・樹形 (ADR-0026、機能追加)

- 樹種マスタは 5 種に削減 (ADR-0026)、それ以外は **カスタム樹種・樹形** をユーザーが作成 (`bonsai_species_custom` / `bonsai_styles_custom`)
- 管理画面: `app/custom-species.tsx` / `app/custom-styles.tsx` (Sess89 で編集・削除 CRUD 完備、master/custom 分離 pattern = R-72)
- Free 上限 3 件 (ADR-0049 ⑥)、Grandfathered 保護あり
- 検索対象に含まれる (§14.3.2)
...
| 盆栽数 1            | 境界     | FlatList 表示                                           |
| 盆栽数 1,000        | 高負荷   | FlatList で遅延なし (60fps 目標)                        |
...
- [ ] 一覧が updated_at 降順で表示される (手動並び替えは未実装)
- [ ] 盆栽 1,000 本登録時も Home スクロール 60fps 維持

### §6.7 対応テスト

- Jest: `__tests__/features/bonsai/` (form utils / card / photo CRUD) + `__tests__/db/` (repository)
- Maestro: `maestro/flows/bonsai-list.yml` / `g4-bonsai-new.yml`
...
SoT は `src/db/schema.ts` の `EVENT_TYPES`（14 種）、UI ラベルは i18n `eventType_*` キー、アイコンは `src/components/icons/EventIcons.tsx`（Lucide 系 SVG、stroke 統一 = Sess92）:

`watering`（水やり）/ `pruning`（剪定）/ `wiring`（針金がけ）/ `unwiring`（針金外し）/ `repotting`（植替え）/ `fertilizing`（施肥）/ `pest_control`（防除・消毒）/ `leaf_trimming`（葉刈り）/ `leaf_first_aid`（葉の手当、ADR-0028）/ `defoliation`（全葉刈）/ `deshoot`（芽かき）/ `candle_cut`（芽切り）/ `moss_care`（苔手入れ）/ `position_change`（置き場変更）

- **Sess16 PR-Q (2026-05-20)**: 旧仕様「candle_cut は松類のみ表示」は撤廃。全盆栽で全種別表示（`isPine` filter 削除）
- 当初案にあった `disease_treatment` / `observation` / `other` は **v1.0 未実装**（v1.x 拡張候補）
- 種別固有 payload は §7.3.2.1 を参照
...
**bulkLogEvents() の入力** (Sess17 PR-H1 拡張): `bonsaiIds` + `type` + `note` + `occurredAtUtc?` + `payload?` (実装の正: `src/db/eventRepository.ts`)。schema 変更不要 (events.payload は JSON、 CHECK 制約なし、 ADR-0028 整合)。 既存 caller は payload 省略で backward-compat。
...
実装の正は `src/db/eventRepository.ts`（`createEvent` / `bulkLogEvents`）。確定画面（WorkLogConfirm / BulkLogConfirm）が保存し、遷移先（予定タブ）が focus 時に再取得する（§5.5）。payload 構築は `buildWorkLogPayload(type, state)` 純関数（§7.3.2.2）。保存成功直後に `maybeRequestReview()`（§5.14、fire-and-forget）。

#### §7.3.4 削除の取り消し（UndoSnackbar、ADR-0036）

記録の削除は即時 `DELETE` ではなく **softDelete（`deleted_at` 付与）+ UndoSnackbar** で可逆化する:

- 削除実行 → `bulkSoftDeleteEvents` + 通知連動 `cancelForEvents`（ADR-0036 D8）→ UndoSnackbar 表示「…を削除しました [元に戻す]」
- [元に戻す] タップ → `restoreEvents` で復元 + reload
- ゴミ箱（`deleted_at IS NOT NULL`）は 30 日で `purgeOldTrash` により物理削除
- 通知の再予約はアプリ起動時に一括実行（ADR-0014 R2）。削除時は該当通知の cancel のみ即時

#### §7.3.5 写真添付（ADR-0049 ③ が正）

- 作業記録画面から複数枚添付可能。**Free = 3 枚/記録、Pro = 10 枚/記録（安全上限 `MAX_PHOTOS_PER_EVENT`）**。表示は全 Free
- 添付済み写真は `photos` テーブルに `event_id` を紐付けて保存
- 基本情報写真（① Free 3 枚/盆栽）とは**独立した記録単位のカウント**（`FREE_PHOTO_LIMIT_PER_EVENT`、photoRepository.ts）。旧構想の「盆栽単位に合算」ルールは ADR-0049 で記録単位に確定し不採用
...
- [ ] 作業記録後、タイムラインに即時反映
- [ ] 削除時に UndoSnackbar 表示、[元に戻す] で復元 (softDelete + restore、ADR-0036)
- [ ] 削除/変換時に該当通知が cancel される (cancelForEvent、再予約は起動時 = ADR-0014 R2)
- [ ] 水やり記録時、盆栽詳細の「最後の水やりから X 日」が reload 後に 0 日へ更新
- [ ] DB 書き込み失敗時、エラートースト表示 + 画面表示は DB と一致 (reload)
- [ ] **(Sess16 PR-Q で撤廃)** ~~松類以外で candle_cut が選択肢に出ない~~ → 全盆栽で全 14 種別表示

### §7.7 対応テスト

- Jest: `__tests__/features/event/` (EventRow / chips / grouping / payload)
- Maestro: `maestro/flows/log-event.yml` / `work-log-edit.yml` / `g3a-bulk-log.yml`
...
#### §12.3.2 針金 event のペイロード（実装の正: `src/db/eventPayloadValidator.ts`）

- wiring: `wire_size_mm` (1/1.5/2/2.5/3 の 5 段階) + `body_part` (all/miki/eda) + `scheduled_unwire_at` (ISO UTC、任意)
- unwiring: `body_part` のみ
...
flowchart TD
  ...
  InsertDB --> Reload[呼び出し元 reload]
```

#### §13.3.2 相対パス保存（basic_spec.md §5.2 重要ルール）

- DB には相対パスのみ保存 (絶対 URI は throw)。読み出し時に `documentDirectory` と結合して絶対 URI 化
- 実装の正: `src/db/filePathUtils.ts` (`toRelative` / `toAbsolute`) — 直接 `FileSystem.documentDirectory + ...` を書かない

#### §13.3.3 リサイズ・圧縮・サムネイル（v1.0 の実態）

- 取り込み時のリサイズ・圧縮・サムネイル生成は **v1.0 未実装**（picker の原本をそのままコピー保存。実装の正は `src/features/photos/photoOrchestrator.ts`）
- 表示側は `expo-image` のキャッシュに任せる（独自サムネイル生成なし）
- リサイズが行われるのは **F-11 バックアップ作成時のみ**（長辺 2048px、§16.7）
- 当初構想（取り込み時 2048px 圧縮 + 300/800px サムネ生成）は将来候補として ADR 起票時に再検討
...
- Jest: `__tests__/features/photos/photoLimit.test.ts` / `photoOrchestrator.test.ts` + `__tests__/photoRepository.test.ts`
- Maestro: `maestro/flows/bonsai-photo.yml`
...
検索クエリは以下 **4 ターゲット**を並列検索し、**Things 3 / Apple Notes 風のグループ表示**で結果を返す (実装: `src/features/search/useBonsaiSearch.ts`):

| ターゲット                                                          | 検索方式                                                              | 想定応答 (25 万行 events) |
| ------------------------------------------------------------------- | --------------------------------------------------------------------- | ------------------------- |
| **盆栽名 (`bonsai.name`)**                                          | LIKE 走査 (200 行レベル、index 不要)                                  | ~1ms                      |
| **樹種名 (`species_names.common_name` 19 言語通称 + カスタム樹種)** | LIKE 走査                                                             | ~5ms                      |
| **樹形 (master enum label + カスタム樹形名)**                       | LIKE 走査 (repository 側 style LIKE、Sess care-search 改修)           | ~5ms                      |
...
実装の正: `src/features/search/useBonsaiSearch.ts` (4 ターゲット並列検索 + アーカイブ/ゴミ箱除外 Y1/Y2 + trim/case-insensitive Y4/Y5) + `src/features/search/searchGrouping.ts` (上限 50 件のセクション配分)。
...
直近 30 日の `bonsai_tags` INSERT 上位 3 件を表示 (実装の正: tagRepository)。
...
- 1 盆栽あたり最大 **10 タグ** (TM1、Bear / Things 業界標準) — **未実装 (Issue #216 で追跡中)**
- アプリ全体タグ最大: **制限なし** (TG1) (性能計測は Issue #1090 PoC で判断)
...
| placeholder 文言 | **「盆栽名・樹種・樹形・メモで検索」** (i18n `searchPlaceholder` が正) | 何が検索できるか明示、19 言語                       |
...
- Jest: `__tests__/features/search/` (searchGrouping / queryHistory / searchHistoryStore / searchHistoryCommit) + `__tests__/db/` (FTS5)
- Maestro: `maestro/flows/search-flow.yml` / `look-back-search-initial.yml`
...
2. **Options Sheet (modal、BottomSheet は ADR-0024 で全廃)**: エクスポート種類タップで開く、盆栽選択 (全件 vs 個別、Y4) + 種類別オプション
3. **Generating Overlay**: 進捗バー + 「キャンセル」ボタン (Y2)
4. **Share Sheet**: iOS / Android 標準共有 UI (OS 委譲)
5. **CSV Preview**: 生成前確認 (実装は `app/export/` 配下が正、mockup v1.10 で簡素化)
...
実装の正: `src/features/export/` の CSV 生成 (自前実装、ライブラリ追加なし。BOM + CRLF + quote escape は `__tests__/features/export/csvExport.test.ts` で担保)。
...
- `BlankPdfError` (1024 byte 未満) / `PdfHangError` (Promise.race timeout) / `PdfStorageLowError` (100MB 不足) — 実装の正: `src/features/export/` (pdfReliability.test.ts で担保)
...
- Free が Hub のエクスポート種別をタップ → 自前 Paywall (`app/pro.tsx`) へ遷移 (`useProStore.isPro` 判定)
- Paywall キャンセル → 無音で戻る (押し付けがましさ排除)
- 当初構想の `react-native-purchases-ui` presentPaywallIfNeeded は不採用 (自前 Paywall = ADR-0009、entitlement は `premium`)
...
必須フィールド: `schema_version` (=1、厳格一致) / `exported_at` (ISO UTC) / `app_version` / `db_sha256` / `stats` (bonsai_count / event_count / photo_count)。実装の正: `src/features/backup/`。
...
#### §17.3.1 言語検出と切替 (実装の正: `src/core/i18n/i18n.ts`)

- 独自 i18n システム (zustand store + AsyncStorage persist、key = `app-i18n-lang`。**i18next は不使用**)
- 初回: `expo-localization` で端末 locale 検出 → 対応 19 言語へマップ (zh は Hant/TW/HK/MO → zhHant、他 → zhHans)、非対応は en
- 切替: Settings → 言語 (`app/settings/language.tsx`)。store 更新で全 `t()` 購読コンポーネントが即時再レンダ (再起動不要)
- fallback: 各 locale ファイルは baseEn を継承しつつ、**全 key 明示定義を `pnpm i18n:check` で強制** (実行時 fallback に頼らない)

#### §17.3.2 複数形・可変表現

- ICU plural / CLDR 形は使わない。可変数値は `{count}` placeholder + 言語別文言設計で表現 (複雑な plural 分岐は文言側で回避)
- 例: ja「{count} 件の作業予定があります」 / en "{count} task(s) scheduled" 相当の言語別表現

#### §17.3.3 日付・数値フォーマット

- `Intl.DateTimeFormat` / `Intl.RelativeTimeFormat` で locale 自動フォーマット (実装の正: `src/core/datetime/`)
- Hermes の Intl 制約に注意 (月名 RangeError 等は fallback 表示)
...
- [ ] 件数表現 ({count} placeholder) が ja/en/pl/ru で自然な文言
...
- Jest: `__tests__/i18n.test.ts` (locale 検出・fallback・key 整合)
- CI: `pnpm i18n:check`, `pnpm i18n:audit`, `pnpm i18n:forbidden`
...
- 設定 → アカウント → プラン確認
- Free 機能タップ時の誘導（例: 写真 4 枚目、エクスポート、定期予定 4 グループ目）。Paywall 実体 = `app/pro.tsx` (自前実装)
...
- Entitlement: `premium` (1 つのみ) / Offering: `default` (1 つのみ)
- Packages: `$rc_monthly` → `bonsailog_pro_monthly`、`$rc_annual` → `bonsailog_pro_yearly`、`$rc_lifetime` → `bonsailog_pro_lifetime` (Non-Consumable / One-time)

#### §18.3.3 購入結果の分類

成功 / キャンセル (無音) / PAYMENT_PENDING (家族承認待ち) / PRODUCT_ALREADY_PURCHASED (復元誘導) / その他エラー — 分類と UI 対応は §18.5 の表が正。実装の正: `src/services/proService.ts` (mapPurchaseErrorCode)。
...
実装の正: `src/stores/proStore.ts` (isPro / planType / expirationDate) + `src/services/proService.ts` (RC SDK ラッパ)。`app/_layout.tsx` で `Purchases.addCustomerInfoUpdateListener` を 1 回だけ登録し、proStore に反映する (Apple Review 3.1.1 / 5.0 準拠)。
...
│  ・広告非表示                │
├─────────────────────────────┤
│  [ 年額 (例 ¥3,980) ] ← 推奨│ Lifetime 所持時は非表示
│   月換算 ¥331/月             │
│  [ 月額 (例 ¥640) ]          │ Lifetime 所持時は非表示
│  [ 買切 (例 ¥9,800) ]        │ Lifetime 所持時は disabled
│  ※金額は例。実価格はストア+RevenueCat が正│
├─────────────────────────────┤
│  [    購読する    ]          │
│  購入を復元 | 利用規約 | プライバシー │
└─────────────────────────────┘
...
- Maestro: `maestro/flows/paywall-to-purchase.yml` / `paywall-tag.yml` / `pro-no-ads.yml`
...
#### §19.3.2 AdBanner の要点 (実装の正: `src/features/ads/AdBanner.tsx`)

- `useProStore.isPro` で Pro は即 null (完全非表示)、`canRequestAds` false も非表示
- 広告ユニット ID は env extras 経由 (直書き禁止)。**デモ ID ⇄ 本番 ID 切替 + 段階ゲート** は Sess95 実装 (adServiceBehavior.test.ts が正)
- iOS バックグラウンド復帰時は 60 秒以内の再ロードをしない (useForeground guard)
- ロード失敗時はプレースホルダ表示
...
## §20. F-15 テーマ (システム / ライト / ダーク)

### §20.1 目的

**3 mode (system / light / dark)** でテーマを切り替える (ADR-0015 + Notes Amended 2026-05-10 PR #312 で **屋外モード (outdoor) は v1.0 不採用・削除済み**。太陽アイコン・previousMode 復帰仕様も同時に消滅)。Free / Pro 全機能利用可。

### §20.2 画面 / 入口

- Settings 内のテーマ選択 (システム / ライト / ダーク)
- チュートリアル中は light 固定 (TT2)

### §20.3 期待動作

#### §20.3.1 テーマ解決 (3 mode)

- `system` → OS の colorScheme に追従 (null は light フォールバック = IM1-A)
- `light` / `dark` → 固定

#### §20.3.2 テーマ解決 + 配信

Tamagui は Phase 7 で撤去済 (ADR-0015 Amendment 2026-05-30)。実装の正: `src/core/theme/themeResolver.ts` (resolveTheme 純関数、`__tests__/core/themeResolver.test.ts`) + `src/core/theme/useColors.ts` (plain hex map) + `src/stores/settingsStore.ts` (themeMode 永続化)。

#### §20.3.3 色トークン

**SoT は `docs/reference/design_system.md` §6 + `src/core/theme/colors.ts`** (本書に hex を複製しない — 旧表は washi 化前の値で drift していたため撤去)。dark は Material 3 baseline #121212 (ADR-0015 BD1)、コントラストは WCAG AA 必須 / AAA 目標 (basic_spec §6.2)。

#### §20.3.4 Reduced Motion 対応 (A1)

OS の「動きを減らす」設定 ON 時はテーマ切替アニメを 0ms にする (標準 200ms。`useReducedMotion` / react-native-reanimated)。

#### §20.3.5 スプラッシュ + ステータスバー (SP1 + SB1)

- `userInterfaceStyle: automatic` (app.config.ts) で OS dark mode 追従
- StatusBar style は resolved theme に応じて自動切替 (実装の正: `app/_layout.tsx`)
...
| 直 hex 違反 (CI 検出) | ESLint エラー (`local/no-color-hex-literal-in-stylesheet`) | コード修正必須、CI block         |

### §20.6 受け入れ条件

- [ ] `src/core/theme/colors.ts` に light / dark の hex マップ (SoT = design_system §6)
- [ ] 3 mode 切替 (システム / ライト / ダーク) 動作、system は OS 即時追従
- [ ] null フォールバック light (IM1-A)
- [ ] dark = Material 3 #121212 (ADR-0015 BD1)
- [ ] スプラッシュ + ステータスバーがモード追従 (SP1 / SB1)
- [ ] AsyncStorage 永続化失敗時無音 (PE1)
- [ ] F-08 写真自体は変更なし、UI 枠のみテーマ追従 (PH1)
- [ ] チュートリアル中は light 固定 (TT2)
- [ ] AdMob 広告は色変えない (Y4)
- [ ] ESLint `local/no-color-hex-literal-in-stylesheet` 等で直 hex 構造防止 (EL1、rule 実体は `eslint-rules/`)
- [ ] 屋外モード / 太陽アイコン / previousMode は **削除済み機能** として復活させない (ADR-0015 PR #312。復活時は ADR 改訂)

### §20.7 対応テスト

- Jest: `__tests__/core/themeResolver.test.ts` + ESLint rule tests (`__tests__/eslint/`)
...
- チャネル 2 種: `bonsai_watering` / `bonsai_daily_summary` (importance = DEFAULT、**権限要求より前に作成**)。実装の正: `src/features/notification/`

#### §21.3.3 スケジューリング擬似コード（7 日ローリング、enforceIosLimit 不要）

- 当日まとめ: identifier prefix `daily_summary_<dateKey>`、既存を全 cancel → planned events を日別集計 → 0 件の日は予約しない (DATE trigger、時刻は Settings 値 default 07:00)
- 水やり: identifier prefix `daily_watering_<h>_<m>`、最大 5 件 (DAILY trigger)
- 再予約は**アプリ起動時のみ** (R2)。実装の正: `src/features/notification/`

**最大通知数**: 水やり 5 件 + 当日まとめ 7 件 = **最大 12 件**（iOS 64 件上限の 1/5、enforceIosLimit ロジック不要）。

#### §21.3.4 通知タップ → Deep Link（作業予定カレンダー S-08）

- 通知タップ → **予定タブ (作業予定カレンダー S-08) を当日選択状態で開く** (内部 router 遷移。完全停止からの復帰は `getLastNotificationResponse` で拾う)。実装の正: `app/_layout.tsx`
...
- **設定状態保持**: `settingsStore` (zustand persist) で永続化、OFF→ON で前回設定復元 (key 実体は `src/stores/settingsStore.ts` が正)
...
- Jest: `__tests__/features/notification/`
- Maestro: `maestro/flows/notification-off-on.yml` / `notification-summary-tap.yml` / `settings-notifications.yml` / `onboarding-notification.yml`

---

## §22. 画面遷移マップ（5 主要フロー）

### §22.1 オンボーディング (実装の正: `src/features/onboarding/onboardingFlow.ts`)

- step は **welcome → language → tut5 (通知)** の 3 step (`ONBOARDING_STEP_ORDER`。当初構想の ATT 説明/UMP/FirstBonsai step と tut1/tut2 は撤去済み)
- ATT / UMP 同意は onboarding step ではなく **広告初期化フロー側** (§19.3.1、app/\_layout) で処理
- 各 step は replace 遷移 (戻る禁止)、全 step 完了/スキップで Home へ

### §22.2 盆栽追加

```mermaid
flowchart TD
  A[Home (tabs)] -->|BottomCtaBar「追加」tap| D[bonsai-new modal]
  D -->|樹種選択| E[species-picker modal]
  E -->|選択| D
...
#### §23.3.5 予定追加導線 + 自動削除 (ADR-0035 D7/D8、FAB は Sess72 で BottomCtaBar に置換 = ADR-0054)

- 画面下部 **BottomCtaBar「予定を追加」** tap → `useBulkActionFlow('schedule')` で予定追加 flow
- 過去日選択時は disable (TEXT_MUTED 灰、押せない、Sess12 PR-D 改善 C)
- **通常導線で記録時の自動削除** (ADR-0035 D7/D8 場面 B): WorkLogConfirm / BulkLogConfirm の通常経路 (= URL param `fromPlannedId(s)` 不在) で書込前に `findPlannedEventByCondition(dateKey, bonsaiId, type)` で同条件 planned を検索 → hit 時 `convertPlannedToRecorded` で atomic 変換 (元 planned 自動 softDelete + 新 logged 作成) + `cancelForEvent` + Toast「予定 {count} 件を記録に変更しました」 (planEventConvertedToast)
...
- 通常導線 (上記 param 不在) → `findPlannedEventByCondition` で auto-cancel 動線 (ADR-0035 D8 場面 B)
...
## §24. Deep Link 仕様 (v1.0 の実態)

- **カスタム URL スキームは未設定** (app.config.ts に `scheme` なし)。外部からの `bonsailog://` 起動は v1.0 非対応 (将来候補)
- アプリ内遷移は expo-router の内部 push で完結。**通知タップ → 予定タブ当日選択** (§21.3.4) が唯一の「deep link 相当」動線
- Universal Link / App Links は v2+ 検討

### §24.3 失敗時の挙動

- 通知 data の日付が不正 → 当日扱いでフォールバック
- 該当日 events 0 件 → 「予定はありません」表示 (§21.5)
...
- **スポットライト**: 画面を暗幕で覆い、本物の UI 要素 1 つを強調 + 吹き出し 1 つ（`src/features/guides/GuideSpotlight.tsx`、対象計測は `useSpotlightTarget` = measureInWindow + Android status bar 補正）
- **お祝いバナー**: 最初の作業記録の保存直後に `GuideBanner`（washi 系 — スポットライトの吹き出しと統一意匠、操作結果 Toast とは分離。`src/features/guides/celebrateFirstRecord.ts`）
- **生涯 1 回**: `src/stores/guidesStore.ts`（AsyncStorage 永続、key = `bonsailog-guides`）。再表示 = 設定 >「使い方」の `resetAll` または **pull 要求**
- **pull 型（#1203）**: 使い方一覧のトピック tap = 実画面に遷移 + `pendingGuide`（揮発 store）経由で対応ガイドを 1 回表示（seen 済みでも表示）。静的な詳細ページは廃止
...
| g2  | 記録タブ初回 open（+ pull）         | `src/features/calendar/CalendarTabScreen.tsx`      |
| g3  | 予定タブ初回 open（+ pull）         | `src/features/calendar/CalendarTabScreen.tsx`      |
| g5  | 作業記録の総件数 0→1 の保存直後     | `BulkLogConfirmScreen` / `WorkLogConfirmScreen`    |
| g6  | 盆栽詳細初回 open                   | `app/(tabs)/bonsai/[id]/index.tsx`                 |
| g7  | pull 専用: 使い方「盆栽を登録する」 | `app/(tabs)/bonsai/index.tsx`（登録 CTA 強調）     |
| g8  | pull 専用: 使い方「定期予定を作る」 | `src/features/recurrence/RecurrenceListScreen.tsx` |
| g9  | pull 専用: 使い方「通知を設定する」 | `app/settings/index.tsx`（scroll-into-view 付）    |
| g10 | pull 専用: 使い方「バックアップ…」  | `app/backup/index.tsx`（作成ボタン強調）           |
| g4  | 欠番（optInPrompt 衝突で不採用）    | —                                                  |

### §26.3 受け入れ条件（要約）

- 各ガイドは自動発火なら生涯 1 回・すべて dismiss 可・1 画面に同時 1 つまで
- g7-g10 は **pull 専用**（自動発火しない — ADR-0058 Amendment 2026-06-12）
...
## §27. 定期予定 (Recurring Schedule、F-02 拡張 — ADR-0056 が正)

### §27.1 概要 (機能追加、Sess78〜)

「毎週月曜に水やり」のような繰り返しルールから planned events を自動生成する。詳細仕様 (D1〜D7) は **ADR-0056 が SoT**、本節は所在の索引のみ。

- **データ**: `recurrence_rules` テーブル (RFC 5545 RRULE、`group_id` で予定グループ化 = Sess99 案 G2)。実装の正: `src/db/recurrenceRuleRepository.ts`
- **作成導線**: 予定追加 flow の 🔁 繰り返し toggle → 6 preset (`src/features/recurrence/RecurrenceFormScreen.tsx`)。次回日付は expandRRule 実計算の SoT 関数 (Sess101 #1157)
- **一覧/編集**: `app/recurring-rules/` + `RecurrenceListScreen` (カードは作業名・頻度が主役、盆栽は件数のみ = Sess101 #1158)。編集は「この 1 件だけ / 以降すべて」等の 3 択 (EXDATE / detached event = ADR-0056 D6)
- **Free 制限**: **3 予定グループまで** (`FREE_RECURRENCE_GROUP_LIMIT`、ADR-0049 ⑦ + Sess101 グループ単位化 #1159)。Grandfathered 保護あり
- **通知連携**: 生成された planned events が F-16 当日まとめ通知に集約される

### §27.2 対応テスト

- Jest: `__tests__/features/recurrence/`
- Maestro: `maestro/flows/recurring-edit-confirm.yml` / `recurring-memo-byday.yml` / `plan-tab-delete-group.yml`

---

## 付録A：小中学生向け用語辞典（本書固有語のみ）

- **ロールバック**: 失敗したら変更を「なかったこと」にする操作 (トランザクション失敗時)。
...
- [ ] 横断仕様（§5）に従っている（特に TXN 選択、書込後 reload）
...
以下は現時点で「決めていない」または「調査中」の項目。実装時に迷ったら ADR を起こすこと (解決済みは打ち消さず転記注記 = 履歴):

1. ~~作業記録の写真のカウント単位~~ → **解決済み**: ADR-0049 ③ で記録単位 (Free 3 / Pro 10) に確定 (§7.3.5)
2. ~~お引っ越しの iOS↔Android クロス~~ → **解決済み**: ZIP + Share Sheet 方式で対応済み (§16.9 受け入れ条件)
3. ~~ヒートマップ Skia 描画性能~~ → **消滅**: F-04 自体が ADR-0039 で撤廃
4. **言語切替中のメモ欄入力中ドラフト**: 切替時に入力中テキストが消える可能性 → 要検証
5. **Lifetime 購入のオフライン挙動**: Offline Entitlements 対象外 (§18.3.7)。RC サーバダウン時の UX 要設計
6. ~~通知許可の二度拒否後の再プロンプト~~ → **解決済み**: 「何もしない (C3)」で確定 (§21.3.1、OS Settings からの自発 ON のみ)
