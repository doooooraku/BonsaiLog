...
- **long-press → 削除動線** (ADR-0035 D3): planned/logged 両 section の EventRow long-press → `Alert.alert(t('planEventDeleteConfirmTitle'), t('planEventDeleteConfirmDesc'), [cancel, delete])` → delete tap で `softDeleteEvent(ev.id)` + `cancelForEvent(ev.id)` + `reload()` (bonsai-detail history タブと同 pattern、 30 日ゴミ箱で復元可)
...
- [ ] EventRow long-press → 確認 Alert → 削除 (planned/logged 両、 ADR-0035 D3)
...
- **ADR-0035**: Phase ε (Sess23、 タブ名 revert + 個別削除 + 予定→記録 atomic 変換 + ふりかえり card 完全 revert)
