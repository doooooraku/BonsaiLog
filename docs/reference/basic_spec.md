...
> **Pro 機能 6 項目の境界 SoT**: [ADR-0049](../adr/ADR-0049-pro-feature-boundary-v1.md) (Sess58 確定、 Sess59 実装着手)。

#### できること

- **Free プラン**: 基本機能、 写真 3 枚/本、 作業記録写真 3 枚/記録、 タグ 3 個、 カスタム樹種樹形 3 件、 エクスポートなし、 バナー広告表示
- **Pro プラン**: 全機能、 写真無制限、 作業記録写真無制限、 タグ無制限、 カスタム樹種樹形無制限、 エクスポートあり、 広告非表示
- 3 つの購入オプション:
  - **月額**: ¥500 / $4.99
...
| プラン | 価格            | 内容                                                                    |
| ------ | --------------- | ----------------------------------------------------------------------- |
| Free   | ¥0 / $0         | 樹木無制限登録、写真 3 枚/本、エクスポートなし、Home 下部バナー広告あり |
| 月額   | ¥500 / $4.99    | 全機能、写真無制限、エクスポートあり、広告なし                          |
| 年額   | ¥3,980 / $39.99 | 全機能、月額比 **33% OFF** 訴求                                         |
| 買切   | ¥9,800 / $79.99 | 全機能、永久、広告なし（Lifetime = **Non-Consumable IAP** として登録）  |

### 8.2 Free / Pro 差分

Pro 機能 6 項目の境界 SoT は **[ADR-0049](../adr/ADR-0049-pro-feature-boundary-v1.md)** を正とする (Sess58 確定、 Sess59 実装着手)。
...
| ③ 作業記録写真 (1 記録あたり、 表示は全 Free) | **3 枚まで**                    | 10 枚まで (実装上限 `MAX_PHOTOS_PER_EVENT`)  |
