...
**BonsaiLog** は、**樹種 × 地域気候 × 作業履歴を、鉢 1 本ずつ一生分記録する、オフライン完結の盆栽台帳**モバイルアプリ。

### 1.2 コア価値（不変条件）

1. **提供価値**: ユーザーは 1 本 1 本の盆栽の樹種・購入日・作業履歴・写真を永続記録し、樹種別の次回作業タイミングを知ることができる。
...
- **U-A（高橋さん 62 歳・日本）**: 父から継承した盆栽 25 本の作業履歴を鉢の前で 1 タップ記録、次回の植替え時期を樹種別に把握する。
...
- **リマインダー (Reminder)**: 次回作業の予定通知。樹種別計算または手動設定で生成される。
...
- リマインダー計算・作業履歴記録・分散アルゴリズム（F-05）は**端末のローカル時刻とタイムゾーン**に依存する。
...
樹種と地域気候に基づき、次回推奨作業日を**決定論アルゴリズム**で計算する。AI 非搭載。

#### できること

- 新規盆栽登録時に**最初の 30 日分のガイドカード**を自動生成（樹種別 + 半球別）
- 作業記録後に**次回推奨日**を再計算
- 推奨日の表示は**幅を持つ**（例: 「4/28〜5/5 に水やり」）
- 根拠の表示（「黒松は春に植替え、芽膨張前が目安」等の**参考情報**テキスト）
- ユーザーによる**推奨日の手動オーバーライド**

#### できないこと（明示）

- **「この樹は病気です」等の診断**（理由: 原則 P2、医療類似リスク）
- **「この値で水やりすべき」等の断定**（理由: 同上。表現は「目安」「参考」「テンプレート」に限定、§11.1 語彙リスト参照）
- **気象 API 連携による天気考慮**（理由: v1 は完全オフライン。気温・降水・湿度を考慮したければ手動記録から）
- **Pro 未契約時の計算結果閲覧**（理由: 課金価値の中核。Free はテンプレート概要のみ閲覧可能、具体的な個別推奨日は Pro）

#### 受け入れ条件（テストで担保）

- `__tests__/domain/schedule/species_timing.test.ts`
- `__tests__/domain/schedule/hemisphere.test.ts`
- `__tests__/domain/schedule/override.test.ts`
- 主要シナリオ: 黒松登録 → 30 日ガイド生成 → 水やり記録 → 次回推奨日更新 → ユーザーオーバーライドで個別上書き
...
同じ週に複数の盆栽の作業が集中しないよう、推奨日を自動で分散する。

#### できること

- 週あたり**作業数の上限**を設定（既定 3 件/週、ユーザー変更可 1〜10）
- 集中検知時に**前後 1 週間ずらす**アルゴリズム
- ずらし結果を**カレンダー UI** で可視化
- ユーザーによる個別ドラッグ調整
- 分散結果の**再計算トリガ**（盆栽追加時 / 作業記録時 / 設定変更時）

#### できないこと（明示）

- **ユーザー意向無視の強制ずらし**（理由: 必ずユーザーが最終調整可能）
- **過去の作業の分散**（理由: 将来予定のみ対象）
- **他ユーザーとの作業時刻シェア**（理由: ユーザー間通信なし）

#### 受け入れ条件（テストで担保）

- `__tests__/domain/schedule/distribute.test.ts`
- `__tests__/domain/schedule/distribute.pbt.test.ts`（Property-Based Testing、fast-check 使用）
- 主要シナリオ: 10 本の盆栽が同週に集中 → 上限 3 件で分散 → すべての週が上限内 → ユーザーが手動調整 → 再計算で他を動かさない

**不変条件（Property-Based Testing で保証）**:

- どの週も作業数 ≦ 上限値
- 純粋関数（`Math.random()` / `Date.now()` 未使用、seed 引数で決定的）
- ユーザー固定済みタスクは移動しない
...
端末のタイムゾーンから気候帯を推定し、推奨作業日の季節を自動調整する。

#### できること

- IANA timezone からの**国コード推定**（`zone1970.tab` ベースの静的テーブル）
- **緯度推定**（国重心データ）
- **半球判定**（北半球 / 南半球）と季節反転
- **気候帯分類**（熱帯 / 亜熱帯 / 温帯 / 冷帯、USDA Zone 簡易推定）
- ユーザーによる**手動オーバーライド**（緯度経度 / 気候帯）

#### できないこと（明示）

- **リアルタイム気象 API 連携**（理由: オフライン原則）
- **GPS による継続位置取得**（理由: プライバシー。初回セットアップと手動変更時のみ取得オプション）
- **マイクロクライメート自動判定**（屋内・ベランダ・屋外の区別は手動タグで対応）

#### 受け入れ条件（テストで担保）

- `__tests__/domain/climate/hemisphere.test.ts`
- `__tests__/domain/climate/tz_to_country.test.ts`
- `__tests__/domain/climate/zone_estimate.test.ts`
- 主要シナリオ: `Asia/Tokyo` → 北半球 → 温帯 → 3 月植替え推奨 / `Australia/Sydney` → 南半球 → 温帯 → 9 月植替え推奨（季節反転）
...
樹種 × 作業種別 × 月 × 半球の 4 次元マトリクス:

- **作業種別**: 13 種（watering, pruning, wiring, unwiring, repotting, fertilizing, pest_control, leaf_trimming, defoliation, deshoot, candle_cut, moss_care, position_change）
- **月**: 1-12（整数）
- **半球**: `north` / `south` / `tropical`（熱帯は季節反転なし）
- **推奨度**: `strongly_recommended` / `recommended` / `optional` / `avoid`

### 10.3 気候判定ロジック

- **IANA timezone → 国コード推定**: `zone1970.tab` ベースの静的テーブル（~40KB、アプリバンドル同梱）
- **国重心から緯度経度推定**: 国コード → 緯度経度の静的テーブル（~5KB）
- **半球判定**: 緯度 ≥ 0 で北半球、< 0 で南半球
- **気候帯分類**: 緯度絶対値で 4 段階
  - < 23.5°: 熱帯
  - < 35°: 亜熱帯
  - < 50°: 温帯
  - ≥ 50°: 冷帯
- **USDA Hardiness Zone**: 低解像度ラスター（~3MB）をバンドル同梱、緯度経度から簡易ルックアップ

### 10.4 データ配布戦略

- **v1.0 初期**: アプリバンドルに prebuild SQLite を同梱（`expo-asset` 経由）
- **更新**: S3 CDN + EAS Update で差分配信（樹種追加・タイミング修正）
- **サイズ目安**: 全樹種 + 名称 19 言語 + タイミング + 気候 = **gzip 後 ~1.2MB**
...
- `src/domain/schedule/distribute.ts`: 分散アルゴリズム（純粋関数）
...
| F-03                 | 🩹1 新規盆栽の 75% が 1 ヶ月で枯死 | Bonsai Empire 公式（業界最大手 540K+ 読者）          |
| F-04                 | 🩹2 健康問題の 98% が水やり由来    | Bonsai Direct 公式                                   |
| F-05                 | 🩹3 リマインダー日付集中           | Bonsai Care App 実レビュー「2/23 と 3/23 に集中」    |
| F-06                 | 🩹4 南北半球・地域差               | Bonsai Care App「南半球設定が逆」                    |
