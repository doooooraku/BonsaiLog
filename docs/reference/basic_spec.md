...
### F-04 水やり履歴の可視化 — **撤廃済 (ADR-0039、 Sess31 PR-B、 2026-05-22)**

> **2026-05-22 撤廃**: 本機能は ADR-0039 で完全削除。 user 判断「履歴可視化は意思決定価値なし」 + ADR-0011 (記録のみ哲学) 整合。
> 削除: ヒートマップ + 月別カレンダー + 日別詳細 modal + `watering-history` 画面 + CareHub「水やり履歴」 card。
> 維持: 「最後の水やりから N 日」 テキスト (盆栽カード上の事実表示)。
>
> 以下の本文は削除前の仕様 (履歴維持、 ADR-0039 が現在の正)。
