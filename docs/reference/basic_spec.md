...
- **F-15 テーマ (システム / ライト / ダーク)**
- **F-16 ローカル通知**
- **F-17 作業カレンダー**（予定 + 記録の月次表示、 PlanScreen 統合カレンダー）
- **定期予定**（F-02 拡張、RFC 5545 RRULE、ADR-0056。Free 3 予定グループ）
- **カスタム樹種・樹形**（ADR-0026、Free 3 件）
- **文脈内ガイド**（F-26、ADR-0058）
...
- 盆栽の編集（全項目）
- 一覧表示は updated_at 降順固定（手動並び替え・ソート切替は v1.0 未実装、将来候補）
...
- 写真の添付（カメラ / ライブラリ）
- **相対パス保存**（§5 重要ルール参照）
- 表示キャッシュは expo-image 任せ（独自サムネイル生成・取り込み時リサイズは v1.0 未実装。リサイズは F-11 バックアップ時のみ）
- 写真の並び替え（手動、photoOrderUtils）
- 写真単体の削除
...
**3 mode (システム / ライト / ダーク)** でテーマを切り替える (ADR-0015。**屋外モード (outdoor) は Notes Amended 2026-05-10 PR #312 で v1.0 不採用・削除済み**)。Free / Pro 全機能利用可。

#### できること

- **システム / ライト / ダーク**の 3 択（既定: システム、null フォールバック light）
- ダークモードは **Material 3 baseline `#121212`** (BD1)。色トークンの SoT は `design_system.md` §6 + `src/core/theme/colors.ts`
- アニメーション 200ms、reduced motion 設定時 0ms (A1)
- F-08 写真自体は変更なし、UI 枠のみテーマ追従 (PH1)
- settingsStore (zustand persist) で永続化
- ESLint (`local/no-color-hex-literal-in-stylesheet` 等、`eslint-rules/`) で直 hex 禁止、useColors 参照強制 (EL1)

#### できないこと（明示）

- **屋外モード (高輝度モード)**（ADR-0015 PR #312 で削除済み。復活は ADR 改訂が必要）
- **ユーザー定義カラーテーマ**（v2+ で検討）
- **画面ごとの個別テーマ**（全画面統一）

#### 受け入れ条件（テストで担保）

- `__tests__/core/themeResolver.test.ts`（mode 解決）+ ESLint rule tests（直 hex 構造防止、`__tests__/eslint/`）
- 主要シナリオ: システム設定ダーク → アプリもダーク / 手動 light 固定 / reduced motion ON → 遷移瞬時
...
- **ダークモード対応**: system / light / dark（F-15。屋外モードは削除済み）
