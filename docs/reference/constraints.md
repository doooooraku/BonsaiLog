...
| ③ 作業記録写真（1 記録あたり、 表示は全 Free） | **3 枚まで**                    | 無制限         | ADR-0049 ③     |
...
> 上記の数値（写真 3 枚 / タグ 3 個 / カスタム 3 件 等）を変更する場合は ADR-0049 の更新が必須。 既存ユーザーの 4+ データは Grandfathered (表示 OK + 削除 OK + 追加のみ Paywall) で保護する (Slack 2022 churn 事件回避)。
