...
### Sess81 Amendment (2026-06-09): IAP 「価格利用不可 + 購入失敗」 修復 + BillingError 導入

**経緯**:
Sess81 でテスター苦情「サブスクの購入はまだ未対応ですかね？」 (= 3 プラン全部「利用不可」 表示 + ボタン tap で「購入に失敗しました」 ダイアログ) を構造調査。 RC MCP で照会したところ:

- ✅ RC Dashboard project `projed4e672d` (BonsaiLog) 完璧構築済 (= apps 3 件 / 6 products / entitlement `premium` / offering `default` is_current=true / Package 3 件全部紐付け済)
- ✅ `.env` の RC API キー (Android `goog_optOZ...lzhM` / iOS `appl_cWMo...QrhB`) は RC Dashboard 値と完全一致
- ❌ **Play Console Android subscription `bonsailog_pro:monthly` / `:annual` の availability.territories が `MN` (モンゴル) のみ** = JP/US 含む 174 国で販売対象外 → `Purchases.getOfferings()` が `current=null` → 価格利用不可 + Package not found

**真因**: Sess47-48 で `google_create_products.py` 実行時、 意図は「USD 基準で全地域自動換算」 だったが、 Google API 側で MN だけが個別 territory entry として登録されてしまった。

**修復**:

1. **Play Console UI 経由** (= user 手作業、 約 10-15 分): Google Android Publisher API は「既存 region の削除」 を拒否するため (= "Cannot remove region once it has been added: MN" / "Regional configs were removed from the base plan: MN" の 400 Bad Request)、 API 経由は不可。 user が Play Console UI から「Subscriptions / In-app products > Pricing & availability > Country/region availability > Select all regions」 を 1 タップ。
2. **アプリ側 UI 改善** (= Sess81 PR、 本 Amendment): `priceUnavailable` / `purchaseFailed` 万能エラーを `offeringsEmpty` 原因別分岐に。 `BillingError` class + `mapPurchaseErrorCode` 拡張で、 24h プロパゲーション中も「ストア準備中」 と前向き表示。

**Sess81 PR コード変更**:

- `src/services/proService.ts`: `BillingError` class export + `PurchaseErrorKind` に `offeringsEmpty` 追加 + `mapPurchaseErrorCode` で `BILLING_OFFERINGS_EMPTY` / `BILLING_PACKAGE_NOT_FOUND` を判定 + `purchase()` で `Error` → `BillingError` throw
- `src/features/pro/PaywallScreen.tsx`: `PURCHASE_ERROR_MESSAGE_KEY` に `offeringsEmpty: 'purchaseErrorOfferingsEmpty'` 追加、 `priceLabel` で priceDetails null 時に `priceUnavailableStorePreparing` 返却
- `src/core/i18n/locales/en.ts` + `ja.ts`: 2 新 key (`purchaseErrorOfferingsEmpty` / `priceUnavailableStorePreparing`)、 他 17 言語は ADR-0033 D1 英訳許容差で en fallback
- `__tests__/services/proPurchaseError.test.ts`: `BILLING_OFFERINGS_EMPTY` / `BILLING_PACKAGE_NOT_FOUND` → `offeringsEmpty` の 3 case 追加、 全 7 → 8 種類網羅 update

**学び (Learned)**:

- **Google Android Publisher API は region の削除を拒否**: 既存購読者保護のため、 `regionalConfigs` 配列から entry を消すと 400。 territory 拡大は「既存 region keep + 新規 region 追加」 のみ。 削減は archive → 再作成必要 (= 既存購読者ゼロでないと不可)。 R-68 候補 (= 外部サービス連携機能の preflight smoke test)
- **`purchaseFailed` 万能エラーの構造問題**: 「壊れている」 印象を 4 ペルソナ全員に与え、 シニア (高橋 62 歳) は「自分が悪い」 と混乱、 米国 IT (Marcus 35 歳) は uninstall 判定。 OWASP 一般エラーメッセージ設計でも「原因を特定できる文言」 が原則。 BillingError 経由で原因別分岐したのが本 Amendment
- **テスト「全 N 種類が網羅」 系の更新忘れ**: 新エラー種別追加時は test L136 等の網羅 assertion を 7 → 8 に同時更新。 静的解析でも検出可能 (= `PurchaseErrorKind` enum と test の数を一致確認)

**残作業 (Sess82)**:

- Phase C = preflight IAP smoke test 配線 + R-68 起票 + ADR-0043 §「スコープ外」 完了化
- iOS 同手順 (= TestFlight 1 週間検証 + Sandbox 6 パターン、 ADR-0009 §Acceptance)
