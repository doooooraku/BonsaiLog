...
- glossary: `docs/reference/glossary.md` (BOM / RFC 4180 / CRLF / WKWebView 制約 / SAF / 3 段階フォールバック / カスタムエラー等 — 追加予定) ※廃止 — Sess101 #1169 で glossary.md 削除 (用語の正 = basic_spec §2 + constraints + コード、翻訳禁止リストは ADR-0033 D3)
...
### Amended（2026-05-29 Sess55、個別 PDF 中間確認画面の廃止 + 生成中オーバーレイの全種統一）

ユーザー UX 改善要望 2 件を `/discuss`（6 名チーム + 4 ペルソナ）で確定し実装。

- **個別 PDF の中間確認画面（`app/export/pdf-preview.tsx`）を廃止**: Sess49 追補3「picker カードタップ
  = 即プレビュー遷移（ラジオ/下部 CTA なし）」+ Sess50「pdf-preview = 軽量確認画面」を見直し。
  picker（`app/export/pdf.tsx`）を **単一選択（カードタップ = ハイライト）+ 下部「出力する」CTA** に変更し、
  CTA で `prepareBonsaiPdf` → `generateBonsaiPdfWithFallback`（3 段階フォールバック）を直接実行 → OS 共有。
  確認画面ファイルとルートを削除。理由: 他エクスポート種別（CSV 3 / list_pdf）が「出力する」で即生成する
  動線と不統一だった。誤タップ耐性は「選択 → 出力」の明示 2 アクションで維持。未使用化した
  `exportPdfConfirmBody` を 19 言語から削除。
- **生成中オーバーレイ（`GeneratingOverlay`）を全エクスポート種が通る唯一の生成中 UI に昇格**: 従来は個別
  PDF（pdf-preview）のみが中央モーダルを出し、CSV 3 / list_pdf は Options Sheet のボタン内インライン
  スピナーのみ、と不統一だった（mockup `export-progress.png` 不一致）。`GeneratingOverlay` に `title`
  （「{種別名} を生成中」、新規キー `exportGeneratingNamed` テンプレ + Hub 種別名キー再利用）・`format`
  （CSV/PDF バッジ。Hub のローカル `FormatBadge` を `ExportFormatBadge` として共通抽出し共用）・
  `showCancel`・`delayMs` を追加し、mockup 準拠（バッジ + 種別名 + spinner + 右下キャンセル）に。
- **遅延表示で「瞬間完了のチラつき」を防止**: `delayMs`（既定 250ms）内に生成が終われば Modal を出さない。
  CSV（即時）は既定 250ms、PDF（数秒）は 0ms で即表示。
- **キャンセルは PDF のみ表示**（ユーザー決定）: 生成開始後は OS 共有を完全には中断できない best-effort の
  ため、即時完了する CSV ではキャンセルを出さず「効かない」誤解を回避。
- **生成を Hub に一元化（二重 Modal 回避）**: リスト系の `runExport` + `GeneratingOverlay` を
  `ExportOptionsSheet` から Hub（`app/export/index.tsx`）に持ち上げ。Sheet は条件を集めて
  `onGenerate(opts)` で返すだけにし、Sheet の Modal の上にさらに Modal を重ねない設計にした。

**恒久策（なぜなぜ分析: 生成中表示の不統一）**: 真因 =「生成中フィードバック」を横断共通部品 + 表示
ポリシーとして定義せず、種類ごとに別々の UI を作っていたこと。対策:

1. `GeneratingOverlay` = 全エクスポートが通る唯一の生成中 UI と本 ADR に明記。
2. 「出す/出さない」は各呼び出し側ではなく overlay 内の遅延しきい値（`delayMs`）で一元判断。
3. 静的解析 test（`exportPreview.test.ts` / `exportGeneratingOverlay.test.ts`）で picker・Hub 双方への
   配線と `pdf-preview` 不在を構造保証。

### Amended（2026-05-30 Sess56、GeneratingOverlay 縦並びでタイトル見切れ解消）

PR #903 の実機検証で `GeneratingOverlay` のタイトルが見切れる事象を確認。「個別盆栽レポート を / 生成中」
「Generating / Individual report…」のように 2 行折り返し時に「中」だけ次行へ孤立する。原因は `headerRow`
が `flexDirection: 'row'` + バッジ 40px + gap 14 で、残り横幅 (~258px) に `title` を `flex:1` で wrap して
いたため。今回 CSV3 種 + list_pdf は遅延表示（250ms）で偶々表示前に完了したが、大量データや遅い端末・
19 言語（特に独/葡/仏）で同じ wrap が必ず発生する構造的問題。

- **`GeneratingOverlay` を「バッジ上＋タイトル下」の縦並びに変更**（`headerCol`：
  `flexDirection:'column' / alignItems:'center' / gap:12`）。`title` から `flex:1` を外し
  `textAlign:'center'` でカード幅をフル活用。`numberOfLines={2}` + `ellipsizeMode='tail'` を保険として
  付与（超長言語でも 2 行で止める）。
- **カード幅上限を 360px → '90%' に拡張**：画面比率で確保し小さい端末でも比率で収まる。`minWidth:260`
  は維持。
- **mockup との関係（R-16）**：`export-progress.png` は横並びだが下書きの一例にすぎず、19 言語×5 種別の
  動的タイトルを構造的に成立させる縦並び案を採用（ADR が正、Design は下書き）。
- **静的保証**：`exportGeneratingOverlay.test.ts` test 2-1 で `headerCol` の `flexDirection:'column'` /
  `numberOfLines={2}` / `ellipsizeMode='tail'` / `textAlign:'center'` / `card.maxWidth:'90%'` を構造アサート。

**恒久策（なぜなぜ分析: タイトル見切れ）**: 真因 =「動的 placeholder (`{name}` 等) を含むタイトルに対する
多言語×バリエーション総当たり視認確認」 の仕組み欠如。対策:

1. **`docs/explanation/design_system.md` 追記**：動的 placeholder を含むタイトル＋アイコンは縦並び・
   `numberOfLines<=2` ・ `ellipsizeMode='tail'` を default とする。
