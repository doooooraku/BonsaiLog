...
### 2026-05-22 Sess29 PR-6 — D2 部分 revert Notes Amended (ADR-0038 D5 連動)

**改訂内容**:

- **D2 Hero 縮小 (container 280→180、 PotIcon 120→100)** を **部分 revert** (Sess29 PR-5)
- 撤回理由: user 真意「画像サイズはそのまま OK」 (Sess29 議題 A 確定)、 ペルソナ 1 (高橋 62 歳) 「写真大きく見たい」 + ペルソナ 3 (プロ 100 鉢) 「識別容易」 両者整合
- 撤回後の設定:
  - container.height: **280px** (Sess28 撤回 → 復元)
  - PotIcon size (placeholder): **120** (復元)
  - overlay (黒帯) height: **64px 維持** (Sess28 確定、 Sess29 でも維持)
  - 盆栽名表示: **名前のみ維持** (Sess28 user 真意 #3 維持)
  - 樹種・樹形表示: **非表示維持** (基本情報タブで参照)
- props 構成 (coverUri + bonsaiName 2 件のみ): **維持** (Sess28 PR-4 dead prop 削除済を継続)
- D1 (KAV 共通 hook) / D3 (バッジ統一) / R-46 (KAV hook 必須) は **引き続き有効**

**関連**: ADR-0038 D5 (本 Notes Amended の親 ADR) / Sess29 PR-5 (BonsaiHero 280 復活) / ADR-0020 Sess28 Notes Amended (Hero 280→180) は本 Notes Amended で連動 revert
