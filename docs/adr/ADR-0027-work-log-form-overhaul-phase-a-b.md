...
### 2026-05-21 Sess19 ADR-0031 起票による Single 動線 DB 書込 path 変更

Sess19 実機検証 (8 試行 100% 再現) で **Single × log mode の DB 書込が完全に失敗** していたことが判明 (本 ADR の Decision §1-3 で確立した form は機能していたが、 保存 path 末端で書込されない状態)。 根本原因は bonsai-detail の useFocusEffect の stale closure (詳細: ADR-0031 §Context)。

ADR-0031 で以下を反映:

- **Single の DB 書込 path 変更**: `WorkLogConfirm.handleSubmit` を旧 `setWorkLogConfirmResult + router.back()` (store-callback) → 新 **直接 await pattern** (`await createEvent + addPhotoFromUri + triggerSummaryReschedule + Toast + router.replace('/(tabs)/plan?selectedDateKey=...')`)
- **bonsai-detail useFocusEffect の log mode consume + persistEventWithPayload は完全削除**、 schedule mode (Case A) のみ維持
- **F-05「気遣い型」 popup** (Sess16 PR-L で bonsai-detail に移植) を WorkLogConfirm に再移植 (closure 整合のため)

本 ADR の Decision §1-2 (14 種別 form 実装 + 共通 field) は引き続き有効、 §5 (複数作業全廃) も維持。 ADR-0031 はその「保存 path の信頼性」 を構造的に担保する関係性。
