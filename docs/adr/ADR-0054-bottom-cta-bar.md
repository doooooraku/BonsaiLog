...
#### D2: Component 設計 = `<BottomCtaBar />`

- **新規 component**: `src/components/common/BottomCtaBar.tsx`
  - props: `label: string`, `onPress: () => void`, `accessibilityLabel?: string` (default = label), `testID: string`, `icon?: React.ReactNode` (default `<PlusIcon size={20} color={ON_BRAND} />`), `showAdBanner?: boolean` (default false), `disabled?: boolean` (default false)
  - hook: `useBottomTabBarHeight()` (常時) + `useSafeAreaInsets()` (常時)
  - style: `position: 'absolute'`, `left: 16`, `right: 16` (full-width margin)、 height 56dp (Material 3 Button タップ領域、 WCAG 2.5.8 ◎)、 BRAND_GREEN bg、 `borderRadius: 28` (pill 型)
  - bottom 計算式: `tabBarHeight + (showAdBanner ? AD_BANNER_HEIGHT_APPROX : 0) + insets.bottom + 0` (画面下端へ密着、 16px 余白なし = bar 自身が CTA 強調)
...
| 4   | 予定タブ (画面下)      | ＋ 一括予定追加 | `planFabLabel`                     | なし (bulk フロー = 一括 を意味的に保持) |
| 5   | 記録タブ (画面下)      | ＋ 作業を記録   | `recordFabLabel`                   | en 未翻訳バグ修正 (現状 ja のまま残存)   |

- **動詞統一方針**: 「**登録 (entity 永続化) / 記録 (event log)**」 の 2 動詞で統一。 予定 = 登録 でシニア層 (高橋 62) の安心感優先 (R-14 配慮)
- **英訳許容差**: `addScheduleCta` ja は「予定を登録」、 en は「Add schedule」 で意訳 (ADR-0033 ターゲット文化への寄せ書き許容)
- **`planFabLabel` 維持**: 予定 tab は date + 複数盆栽の **bulk フロー** を起動するため、 「一括」 prefix が意味的に正しい (Sess12 確立)
