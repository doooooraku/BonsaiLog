...
## Notes Amended (Sess106、2026-06-13): Settings での Banner 配置禁止 + AdBanner stacking pattern

Sess106 sprint Phase 2 (PR-6 Settings ProBanner + PR-7 plan/record AdBanner 展開) のための Layout Contract 拡張。

### Notes 1: Settings 画面で bottom fix banner 禁止 (= sticky header 型推奨)

- ADR-0061 (Pro CTA Banner、Sess106 起票) で Settings 内に Pro Banner を新設、配置は **sticky header (画面上部 fix)** を採用
- Settings 画面に **bottom fix banner (例: 「アカウント削除」 等の destructive CTA)** を追加することは **禁止**
- 理由: 将来 Settings に何らかの操作 CTA を bottom 配置すると ADR-0061 ProBanner + 操作 CTA + AdBanner (Free 時) + TabBar の 4 段重ねが発生、視覚混乱 + tap conflict
- 該当 Issue 発生時は **画面分割 / modal 化 / inline 化** で対応 (= bottom fix は禁止)

### Notes 2: AdBanner + BottomCtaBar stacking pattern (Z-order)

ADR-0010 Sess106 Amendment で AdBanner が plan/record タブにも展開 (PR-7 = Issue #1252) されるため、BottomCtaBar との配置 contract を明文化:

#### Z-order (画面下部から上へ)

```
┌─────────────────────────┐
│ TabBar (R-Native)       │ ← 最下層 (Native nav)
├─────────────────────────┤
│ AdBanner (Free 時のみ)   │ ← TabBar の上に inline 配置
├─────────────────────────┤
│ BottomCtaBar             │ ← AdBanner の上に inline 配置
├─────────────────────────┤
│ ScrollView / FlatList    │ ← 自然と BottomCtaBar の上で終わる (R-62)
└─────────────────────────┘
```

#### JSX 配置順序 = Z-order の逆 (上から書く)

```tsx
<ThemedView style={{ flex: 1 }}>
  <SearchHeader />
  <ScrollView>{/* content */}</ScrollView>
  <BottomCtaBar /> {/* AdBanner より先に書く */}
  <AdBanner /> {/* TabBar に最も近い位置 */}
</ThemedView>
```

#### 禁止事項

- ❌ AdBanner を ScrollView 内に配置 (= R-62 Layout Contract 違反、scroll に追従されて期待挙動と異なる)
- ❌ AdBanner を `position: absolute` で配置 (= flex layout の自然 stacking と衝突)
- ❌ BottomCtaBar の **下** に AdBanner を配置 (= 親指リーチ範囲を広告が占有、誤タップリスク激増)
- ❌ guide spotlight 測定 wrapper (collapsable=false) の中に AdBanner を配置 (= onboarding spotlight が AdBanner も囲む UI バグ、ADR-0058 整合)

### R-62 拡張 (Sess106): Multi-surface Layout Contract

R-62 (Component SoT + Layout Contract SoT) に「Multi-surface stacking contract」を追加:

- 画面下部に複数 surface (BottomCtaBar / AdBanner / TabBar) が並ぶ場合、**Z-order と JSX 順序の SoT** を本 ADR-0054 Notes 2 に集約
- 新規画面で同パターン採用時は、本 Notes 2 の図解コードをコピー (drift 防止)

**関連 ADR**: ADR-0010 (Sess106 Amendment、AdBanner 3 タブ展開) / ADR-0061 (Sess106 起票、Pro CTA Banner sticky)
**関連 Issue**: #1252 (PR-7) / #1251 (PR-6) / #1249 (PR-4 = 本 Amendment 起票)
