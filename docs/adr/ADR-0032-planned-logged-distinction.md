...
### 2026-05-21 Sess22 ADR-0034 整合で D1 アイコン併用に拡張

WCAG 1.4.1 (Use of Color、 Level A) 違反 = 「色のみ」 で planned/logged を区別すると色覚多様性 (赤緑色覚異常 = 男性 8%) + シニア老眼 + 屋外モード視認性で識別不能。 業界 1 次情報 ([UX Patterns Dev](https://uxpatterns.dev/patterns/data-display/calendar): 「Do not rely on color alone to convey severity, completion, or selection state. Should be backed by pattern or icon differentiation for color-blind users」) 整合で ADR-0034 D3 で **色 + アイコン併用** に拡張:

- **logged**: BRAND_GREEN **filled** 円 (●、 既存色維持)
- **planned**: ACCENT_BARK **outline** 円 (○、 既存色維持 + borderWidth 1px outline 形状追加)
- 凡例 (Legend、 ADR-0034 D1) で「● 完了 / ○ 予定 / + 複数作業」 説明文表示、 color-blind ユーザーも文字 + 形状で識別可
