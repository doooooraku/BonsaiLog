...
### Notes Amended (2026-05-27、 Sess49 追補3): ふりかえり Hub に「エクスポート」 5 カード目追加

F-10 エクスポート (CSV/PDF) の動線が設定画面からのみで発見性が低い問題に対し、 ふりかえり
Hub (`app/(tabs)/look-back/index.tsx`) に **「エクスポート」 card を追加** (3→4 表示 card、
針金がけ一覧 / 盆栽を検索 / タグを管理 / **エクスポート**)。設定 row も併存維持。

- Pro 限定機能のため card に **PRO バッジ**を表示 (押下後 Paywall の混乱防止)。
- アイコンは新規 `DownloadIcon` (NavIcons.tsx、 トレイ + 下向き矢印)。
- 「ふりかえり = 振り返り + 整理 + 持ち出し」 と意義拡張 (Sess9 のタグ管理追加と同じ拡張路線)。
- i18n: `lookBackCardExportTitle` / `lookBackCardExportDesc` 追加。`/export` Hub へ遷移。

### Notes Amended (2026-06-10、 Sess92): 盆栽タブ + ふりかえり hub 樹種/樹形 card icon 差替

Sess89 Phase 2/3 でカスタム樹種 / 樹形管理 card を hub に追加した際、 既存 icon を 2 重用途で
流用したため意味整合が崩れていた問題に対し、 **3 icon 同時差替** で構造解消 (= Sess36
ADR-0042 D5「TabBar 記録タブ icon 差替」 と同 pattern、 ADR-0042 D4 lint 整合維持)。

#### 改訂内容

| 変更先                                     | 旧 icon                                                             | 新 icon                                                                     |
| ------------------------------------------ | ------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| TabBar 盆栽タブ (`_layout.tsx` L60、 28px) | `LeafIcon` (= hub 樹種 card と 2 重)                                | **`BonsaiIcon`** (= 鉢 + 樹冠 + 幹 silhouette、 新規 NavIcons.tsx)          |
| ふりかえり hub「樹種を管理」 card (22px)   | `LeafIcon`                                                          | **`SproutIcon`** (= Lucide sprout 公式 path、 双葉新芽、 新規 NavIcons.tsx) |
| ふりかえり hub「樹形を管理」 card (22px)   | `CompassIcon` (= `position_change` event と 2 重、 方位磁石 ≠ 樹形) | **`StyleIcon`** (= 文人木風 樹形 illustration、 新規 NavIcons.tsx)          |

#### 真因 (= 5 Whys 抜粋)

- LeafIcon (TabBar 用) と CompassIcon (event 用) を流用 → icon semantic uniqueness の self-check 不在
- 既存 `pnpm verify:icon-duplication` は **同名 export 重複** のみ検出、 **同 component の 2 重 意味用途** は検出範囲外
- 4 ペルソナ全員 ✕ (= 葉 ≠ 種類、 方位磁石 ≠ 樹形)

#### 関連

- `src/components/icons/NavIcons.tsx` (BonsaiIcon / SproutIcon / StyleIcon 新規 3 関数追加)
- `src/components/icons/index.ts` (barrel alphabetical 追記)
- `app/(tabs)/_layout.tsx` (TabBar 盆栽タブ icon 差替)
...
### Notes Amended (2026-06-11、Sess99): 盆栽詳細「作業予定」タブ timeline UI → EventRow detailed Card 統一 (#1120)

**経緯**: Sess93 PR-6 (#1062) でカレンダー画面の予定 section を EventRow detailed Card に統一した際、本タブ (BonsaiTimelineTab) は「ADR-0020 改訂を伴う scope のため follow-up」と deferred (= ADR-0056 §Sess93 v1.0.4 持ち越し PR-7)。Issue 未起票で追跡が消失し、user が v13 実機検証で「作業予定画面が Card 化されていない」と指摘 → Issue #1120 起票 + 本 Amendment で実装。

**決定**:

1. **mockup `bonsai-detail-timeline-01/02.png` の独自 timeline UI (縦線 + 緑円 + 「今日」起点) を本タブで廃止**し、作業履歴タブ (BonsaiHistoryTab) と同型の表示に統一する。mockup より「作業記録と同じ Card」(= Sess93 user 要望、決定 7 の拡張) を優先 (R-16: ADR が正)。
2. **表示構成**: 連続日 group = 履歴タブ同型 group header (EventIcon box + ×N badge + 日付範囲 + 「個別に開く ▼」 toggle + 展開時 縦線 + EventRow detailed) / 単発 = EventRow displayMode="detailed" Card (作業名 + 日付 + memo 3 行 + もっと見る)。status='planned' は写真 strip 自動非表示 (#1062 整合)。
3. **「今日」起点 row + 大円マーカーは廃止**: section title「これからの予定」+ 各 Card の日付表示で起点が自明のため。i18n key `detailTimelineToday` は残置 (削除は i18n 棚卸しで判断)。
4. **操作 (kebab / 削除 / tap) は本タブでは従来どおり非配置** (= 表示統一のみ、挙動不変)。操作追加はカレンダー画面の planned 操作 (#1062 系) との整合判断を伴うため、必要になった時点で別 Issue。

**関連**: Issue #1120 / ADR-0056 §Sess93 v1.0.4 持ち越し + §Sess99 Amendment / ADR-0041 (EventRow displayMode matrix)

---

### Notes Amended (2026-06-12): 機能チュート撤去 (v1.x-2) の補完 = 文脈内ガイド (ADR-0058)

v1.x-2 (2026-05-16) の機能チュート全撤去判断は**維持** (スライド型は今後も不採用)。撤去時に未補填だった「発見させる仕組み」側を、**文脈内ガイド (ADR-0058、Issue #1178)** が補完する — 教えたいことを使う瞬間の画面の中に置く方式 (スポットライト + Toast 流用、生涯 1 回)。1 次情報根拠 (NN/g 70 名実験 / Apple HIG / Material) と設計原則 5 箇条は ADR-0058 が正。

**関連**: ADR-0058 / ADR-0018 Notes Amended (2026-06-12) / Issue #1177-#1179
