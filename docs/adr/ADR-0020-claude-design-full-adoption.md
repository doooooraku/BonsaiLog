...
| 16  | `care-screens.jsx SearchScreen` (mockup v1.0)                                   | 10 SEARCH (探すタブ)              | `app/(tabs)/find/index.tsx` (既存 `app/search.tsx` 廃止統合)                                                                                | 部分整合済 (PR #337 / PR #338、設定アイコン削除、Search-as-Header + live search + tag filter は Issue #339)       |
