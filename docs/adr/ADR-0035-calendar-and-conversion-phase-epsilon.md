...
1. ~~**Maestro flow 残 5 件追加**~~ — Sess24 Phase ζ-1 着手中 (PR-ζ-1-① 等で順次)、 一部 Phase η に持ち越し
2. ~~**unit test 13 case 追加**~~ — **Sess24 PR-ζ-1-② (#732) で完遂** (静的解析 13 case PASS)
3. ~~**PlanScreen unused styles cleanup**~~ — **Sess24 PR-ζ-1-③ (#731) で完遂** (7 styles + bulkLogEvents import 物理削除)
4. **~~SUMMARY 通知 identifier に event ID prefix~~** — **Sess24 PR-ζ-1-④ で方針修正**:
   - 旧案 (event ID prefix) は ADR-0014 「1 通知 = 件数集計」 設計と矛盾と判明
   - 新案: `cancelForDateKey(dateKey, t)` helper を新設 (signature 整備のみ)、 内部実装は `triggerSummaryReschedule(t)` wrap 維持
   - 真の dateKey 単位最適化 (該当 identifier 1 件 cancel + 1 件 re-schedule) は **Phase η Future Work** に持ち越し
5. **`scripts/eslint-rules/atomic-business-tx.mjs`** で R-43 違反 (transaction 外の DB 連鎖呼出) を AST grep で自動検出 — 未着手

### Sess24 Phase ζ-1 達成 (2026-05-21)

| PR    | Title                              | 達成                                            |
| ----- | ---------------------------------- | ----------------------------------------------- |
| #731  | PR-ζ-1-③ unused styles cleanup     | 7 styles + 1 import 物理削除                    |
| #732  | PR-ζ-1-② unit test 13 case         | 13/13 PASS (静的解析 + signature 整合)          |
| #(本) | PR-ζ-1-④ cancelForEvent 設計再評価 | `cancelForDateKey` helper 新設 + JSDoc 方針修正 |

Phase ζ-1 で残 Future Work #1 (Maestro 5 flow) + #5 (ESLint rule) は Phase η/v1.x に持ち越し。

### 2026-05-21 Sess25 Phase ζ-2 — D3 拡張 Notes Amended (ADR-0036 起票)

ADR-0036 (破壊的操作 UX 統一 pattern) を本日起票、 ADR-0035 D3「PlanScreen long-press 削除」 を以下の通り拡張 (10 PR、 user 全 A 承認、 4 ペルソナ ✕ なし全員一致):

**D3 拡張内容** (ADR-0036 D1-D9 参照):

| 旧 D3 (Sess23)                  | 拡張後 (Sess25 ADR-0036)                                                 |
| ------------------------------- | ------------------------------------------------------------------------ |
| 個別 EventRow long-press のみ   | + group 行 long-press + kebab menu (⋮) 3 動線                            |
| `Alert.alert` OS 標準           | `<ConfirmDialog>` カスタムモーダル (アプリ世界観統一)                    |
| planned/logged 統一文言「予定」 | planned/logged 文言分離 (「予定」 / 「記録」 + single/group/wiring 文脈) |
| desc「30 日後自動消去」         | desc 削除 (即削除真意、 復元動線は別途)                                  |
| 削除後 reload のみ              | + `<UndoSnackbar>` 4s 表示 + `restoreEvents` で復元可能                  |
| Haptics fb なし                 | + `expo-haptics` 触覚 fb (長押し成功 + 削除実行)                         |
| wiring cascade 未考慮           | + wiring planned 削除 → 対応 unwiring scheduled 通知 自動 cancel         |
| (改善 ② なし)                   | + EventRow `showBonsaiName=true` 時 作業名 + 日付 重複 物理削除          |

**実装 PR**: PR-ζ-2-① 〜 ⑩ (10 PR 1 セッション、 ADR-0036 Implementation 章参照)

**新規ルール起票** (本 D3 拡張に伴う恒久策):
