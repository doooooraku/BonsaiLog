...
> 起票: Phase 3 Step 3(大規模リファクタ安全網)。境界 ESLint plugin は Phase 6 で error 化予定。
...
- Phase 1 調査で **既知の境界違反 7 件**(`core→stores` ×3 / `db→services` ×2 / `db→features` ×1 / `stores→services` ×1)を検出済み。
- これらは **Phase 6**(FSD 境界整理)で是正し、`eslint-plugin-boundaries` を warning → error に昇格して恒久化する。
- それまでは boundary plugin は **OFF**。本書の依存ルールは「目指す姿」であり、既存違反は段階的に解消する(ADR-0046「FSD 層定義」で正式化予定)。
...
4. **Phase 6(境界整理)**: §2 の違反 7 件を是正、boundary plugin を error 化。
