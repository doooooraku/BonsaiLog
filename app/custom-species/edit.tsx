/**
 * カスタム樹種 編集 / 追加 全画面 (Sess89 Phase 2、 ADR-0049 ⑥ Grandfathered 緩 削除 OK の構造実装)。
 *
 * 動線:
 * - 管理画面 → 「+ 樹種を追加」 → /custom-species/edit (add mode)
 * - 管理画面 → row tap or kebab 「編集」 → /custom-species/edit?id=xxx&initialName=yyy (edit mode)
 *
 * 模倣 pattern:
...
const SPECIES_NAME_MAX_LENGTH = 64;

export default function CustomSpeciesEditScreen() {
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      try {
        const count = await countBonsaiByCustomSpecies(id);
  ...
  const performRename = async (trimmed: string): Promise<void> => {
    const result = await renameCustomSpecies(id!, trimmed);
    if (result === 'duplicate') {
      Alert.alert(t('error'), t('customSpeciesRenameDuplicateBody'));
      setBusy(false);
      return;
    }
    if (result === 'empty') {
      Alert.alert(t('error'), t('customSpeciesAddFailedBody'));
      setBusy(false);
      return;
    }
    router.back();
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isEditMode) {
        ...
        const canCreate = await canCreateNewCustomSpecies(trimmed, isPro);
        if (!canCreate) {
          Alert.alert(
            t('customLimitTitle'),
        ...
        await createOrFindCustomSpecies(trimmed);
        router.back();
      }
    } catch {
      Alert.alert(t('error'), t('customSpeciesAddFailedBody'));
  ...
  const submitLabel = isEditMode ? t('customSpeciesEditUpdateCta') : t('customSpeciesEditAddCta');

  const deleteDialogTitle = t('customSpeciesDeleteConfirmTitle').replace('{name}', initialName);
  const deleteDialogBody =
    usageCount > 0
      ? t('customSpeciesDeleteImpactBody').replace('{count}', String(usageCount))
  ...
  return (
    ...
    >
      <Stack.Screen
        options={{
          title: isEditMode ? t('customSpeciesEditTitleEdit') : t('customSpeciesEditTitleAdd'),
        }}
      />
      <View style={styles.content}>
        <ThemedText style={[styles.label, { color: c.textSecondary }]}>
          {t('customSpeciesEditNameLabel')}
        </ThemedText>
        <View style={styles.inputWrap}>
          <TextInput
            accessibilityLabel={t('customSpeciesEditNameLabel')}
            ...
            placeholder={t('customSpeciesAddPlaceholder')}
          ...
          <ThemedText style={[styles.counter, { color: c.textSecondary }]}>
            {input.length}/{SPECIES_NAME_MAX_LENGTH}
          </ThemedText>
        </View>
        ...
        {isEditMode && (
          <Pressable
            accessibilityRole="button"
            accessibilityLabel={t('customSpeciesEditDeleteCta')}
