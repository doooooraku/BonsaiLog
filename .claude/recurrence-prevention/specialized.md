...
- 親ファイル: `.claude/recurrence-prevention.md` (R-1 〜 R-12 全文 + R-13 以降の索引 + 運用ルール)
...
### Sess104 拡張 (#1236 — worktree agent 起動が hooksPath を戻す事象への自動修復)

Sess104 (2026-06-12) で worktree 隔離 agent (Agent tool, isolation:"worktree") の起動後に `core.hooksPath` が `.git/hooks` へ戻る事象が **1 日 3 回再発** (CLAUDE.md §9 のしきい値到達)。対策: `.claude/settings.json` の **PostToolUse(Agent|Task) hook** で `check-hooks.mjs --quiet` (auto-fix) を自動実行。修復は hook、auditing は従来どおり `verify:hooks` (--check) の 2 層。効果確認は次回 retro で「hooksPath ずれ 0 回」。
