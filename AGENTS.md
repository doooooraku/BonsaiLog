...
- R-13 以降: 専門ルール (詳細は `recurrence-prevention/specialized.md` を参照)
  - 例: R-13 (議論モードでの質問数・ラウンド数の予告) / R-22 (Codex 不採用 + docs-lint 構造防止) / R-44 (破壊的アクション pattern) / R-57 (PR マージ時にブランチを都度削除)

**役割分担**:

- `docs/reference/tasks/lessons.md` = 技術 lesson（DB / Backup / ビルド等の領域別）
...
This project is run by **Claude Code alone** (Codex は使用しない、2026-05-01 ADR で確定)。Each W step has a clear purpose (see `docs/how-to/workflow/whole_workflow.md`):
