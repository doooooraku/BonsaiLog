...
### 先送り照合（deferred scope、Sess99 恒久策）

> **背景 (Sess99 / Issue #1119-#1124)**: Sess93 の follow-up 送りが Issue 未起票のまま v13 に
> 未搭載となり、user が実機検証で「直したはずが直っていない」と発見した。リリース前に
> 「このビルドに入っていないもの」と user / テスターの期待のズレを必ず照合する。

- [ ] **直近セッションの先送り項目を棚卸し**: `gh issue list --state open` + 直近 ADR の「持ち越し / deferred / follow-up」記載を grep し、今回ビルドに **入っていない** user 期待項目を列挙
- [ ] ビルド commit 確定後、`git log <前回ビルド commit>..<今回ビルド commit> --oneline` で **入った変更の一覧**を作る
- [ ] リリースノート / テスター周知文に「**今回入っていない既知の改善**（次バージョン予定）」を 1 行以上明記（該当なしならその旨）
