...
#### 1.5 先送り (deferred scope) 棚卸し — Sess99 恒久策

> **背景 (Sess99 / Issue #1119-#1124)**: Sess93 の「follow-up 送り」が Issue 未起票のまま
> 追跡消失し、user が v13 実機検証で「直っていない」と発見した。先送りの記録先が
> PR 本文 / ADR のみだと誰も追跡しない — セッション終了時に必ず棚卸しする。

今セッションの会話ログ + 作成した PR 本文から **意図的に先送りした項目** を抽出する:

- キーワード: 「follow-up」「先送り」「持ち越し」「deferred」「後日」「次セッション」「scope 外」
- 今セッションの PR は `gh pr list --search "author:@me" --state all --limit 10` で取得し本文を確認

抽出した各項目について:

- [ ] **Issue # が存在するか確認** (`gh issue list --search "<キーワード>"`)
- [ ] Issue が無い先送りは Phase 2 で ⚠️ 警告し、**Issue 起票を提案に含める**
- [ ] user が議論で要望した内容の先送りは、Phase 2 の提示で **「やらなかったこと」として明示**する
...
- [発見1]
- [発見2]
- [アドリブ成功 (計画外でうまくいった工夫) があれば 1 件以上含める。仕組み化候補なら実装先 (file/Issue) を 1 行付す — /retro Step 5 アドリブ成功レンズの毎セッション入口、Sess103 由来]
...
### ⏸ 先送り項目（deferred scope、Sess99 恒久策）

[今セッションで先送りした項目の一覧。各項目に Issue # を併記。
Issue 未起票の先送りがあれば ⚠️ + 起票提案。なければ「なし」]
